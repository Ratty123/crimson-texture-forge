    ...
    *,
    ...
    with urllib.request.urlopen(request) as response, destination.open("wb") as handle:
        ...
        while True:
            ...
            if on_log and (downloaded >= next_report or (total_bytes and downloaded == total_bytes)):
                if total_bytes:
                    percent = int((downloaded / total_bytes) * 100)
                    on_log(f"Downloaded {downloaded / (1024 * 1024):.1f} MiB / {total_bytes / (1024 * 1024):.1f} MiB ({percent}%).")
                else:
                    on_log(f"Downloaded {downloaded / (1024 * 1024):.1f} MiB...")
                next_report += chunk_size * 8
    return destination


def discover_chainner_windows_portable_url() -> str:
    request = urllib.request.Request("https://chainner.app/download/", headers={"User-Agent": APP_NAME})
    html = urllib.request.urlopen(request).read().decode("utf-8", "replace")
    match = CHAINNER_WINDOWS_PORTABLE_RE.search(html)
    if not match:
        raise ValueError("Could not find the Windows portable chaiNNer download on the official download page.")
    return match.group(0)


def download_chainner_portable(
    install_dir: Path,
    *,
    on_log: Optional[Callable[[str], None]] = None,
) -> Path:
    ...
    download_url = discover_chainner_windows_portable_url()
    if on_log:
        on_log(f"Found chaiNNer portable package: {download_url}")

    temp_zip = Path(tempfile.mkdtemp(prefix="crimson_texture_forge_chainner_dl_")) / "chainner_portable.zip"
    try:
        copy_url_to_file(download_url, temp_zip, on_log=on_log)
        if on_log:
            on_log(f"Extracting chaiNNer into {install_root}")
    ...
    candidates = sorted(install_root.rglob("chaiNNer.exe"))
    if not candidates:
        raise ValueError("Downloaded chaiNNer package did not contain chaiNNer.exe.")
    return candidates[0]


def discover_latest_texconv_asset_url() -> str:
    request = urllib.request.Request(DIRECTXTEX_RELEASES_API_URL, headers={"User-Agent": APP_NAME})
    releases = json.load(urllib.request.urlopen(request))
    if not isinstance(releases, list):
        raise ValueError("Unexpected response from DirectXTex releases API.")
    ...
    for release in releases:
        ...
        for asset in assets:
            ...
            if name.lower() == "texconv.exe":
                url = asset.get("browser_download_url")
                if isinstance(url, str) and url:
                    return url

    raise ValueError("Could not find a texconv.exe asset in the official DirectXTex releases.")


def download_texconv_executable(
    destination: Path,
    *,
    on_log: Optional[Callable[[str], None]] = None,
) -> Path:
    download_url = discover_latest_texconv_asset_url()
    if on_log:
        on_log(f"Found texconv.exe release asset: {download_url}")
