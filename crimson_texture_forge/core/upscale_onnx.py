...
def _run_single_onnx_attempt(
    ...
    *,
    ...
    if not plan_entries:
        if on_log:
            on_log("No PNG files require ONNX Runtime processing under the current plan; skipping backend stage.")
    ...
    for index, input_png in enumerate(png_inputs, start=1):
        ...
        if plan_entry is None:
            raise ValueError(f"Missing planner entry for ONNX input: {rel_display}")
        ...
        if plan_entry.action == "upscale_then_rebuild":
            ...
            raise ValueError(f"Planner selected unexpected ONNX action for {rel_display}: {plan_entry.action}")
...
def run_onnx_stage(
    ...
    *,
    ...
    attempt_tiles = (
        candidate_tiles if config.retry_smaller_tile_on_failure and not config.dry_run else (retry_plan.requested_tile_size,)
    )
